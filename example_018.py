...
def PacketHandler(pkt):
    ...
    if pkt.haslayer(WPA_key):
        ...
        if apbssid and bssid != apbssid.upper():
            print(f'WPA handshake packets of other AP: {bssid} from '
                  f'station: {station}')
            return

        if station not in wpa_handshakes:
            wpa_handshakes[station] = \
                {'ts': time.time(), 'frame2': None, 'frame3': None,
                 'frame4': None, 'replay_counter': None, 'packets': []}
        else:
            if time.time() - duration > wpa_handshakes[station]['ts']:
                wpa_handshakes.pop(station, None)  # Eliminate interiors
                wpa_handshakes[station] = \
                    {'ts': time.time(), 'frame2': None, 'frame3': None,
                     'frame4': None, 'replay_counter': None, 'packets': []}
                if verbose > 1:
                    print("Resetting time for station %s" % station)
        ...
        # check for frame 3
            ...
            wpa_handshakes[station]['replay_counter'] = replay_counter  # store the replay counter
            wpa_handshakes[station]['packets'].append(pkt)

        # check for frame 4
        elif (key_info & WPA_KEY_INFO_MIC) and (key_info & WPA_KEY_INFO_ACK == 0) \
                and (key_info & WPA_KEY_INFO_INSTALL == 0) and \
                wpa_handshakes[station]['replay_counter'] == replay_counter:
        ...
        if wpa_handshakes[station]['frame2'] and wpa_handshakes[station]['frame3'] and \
                wpa_handshakes[station]['frame4']:
