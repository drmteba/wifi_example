...
########################################
def get_mon_iface(arg):
    global monitor_on
    monitors, interfaces = iw()
    if arg.interface:
        monitor_on = True
        monmode = start_mon_mode(arg.interface)
        return monmode
    if len(monitors) > 0:
        monitor_on = True
        return monitors[0]
...
def iw():
    ...
    interfaces = []
    try:
        outputs = subprocess.run(["iw", "dev"],
                                 capture_output=True, text=True)
        pattern = re.compile(r'type (\w+)+', re.I)
        pattern2 = re.compile(r'Interface (.+)+', re.I)
        ifac = pattern.search(outputs.stdout)
        interface = pattern2.search(outputs.stdout)
        if ifac.group(1) == 'monitor':
            monitors.append(interface.group(1))
        elif ifac.group(1) == 'managed':
            interfaces.append(interface.group(1))
        else:
            print('no wireless car available')
    except:
        sys.exit('Could not execute iw')
...
def channel_hop(mon_ifac, arg):
    ...
    channelNum = 0
    maxChan = 11 if not arg.world else 13
    err = None

    while 1:
        if arg.channel:
            with lock:
                monchannel = arg.channel
        else:
            channelNum += 1
            ...
            try:
                proc = subprocess.run(['iwconfig', mon_ifac, 'channel', monchannel],
                                      check=True, capture_output=True, text=True)
            except OSError:
                print(('[' + R + '-' + W + '] Could not execute "iw"'))
                os.kill(os.getpid(), SIGINT)
                sys.exit(1)
            if len(proc.stderr) > 2:  # iw dev shouldn't display output unless there's an error
                err = '[' + R + '-' + W + '] Channel hopping failed: ' + R + proc.stderr + W
