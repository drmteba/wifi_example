...
from scapy.layers.dot11 import Dot11Deauth, Dot11AssoReq, Dot11Auth, Dot11ProbeResp, Dot11

intfmon = 'wlan0'  # monitor mode interface
intfmain = 'wlan0'  # interface in managed mode connected to Wi-Fi APs
...
[x.upper() for x in macprotectlist]  # convert all MAC addresses to uppercase
...
def PacketHandler(pkt, lastproberesp=None):
    global lastdeauth, lastassoc, lastauth, pktssaved, duration, savecap, risk

    if pkt.haslayer(Dot11Deauth):
        if (pkt.addr1.upper() in macprotectlist or pkt.addr2.upper() in macprotectlist
            or pkt.addr3.upper() in macprotectlist) and \
                time.time() > (lastdeauth + duration):
            lastdeauth = time.time()
            if pkt.sprintf("%Dot11Deauth.reason%").startswith('class3-from-nonass'):
                risk = "RISK: HIGH!\n"
            ...
            if verbose:
                print(message)
            if logging:
                slogger.critical('WIFIMON:' + message)
            if savecap:
                try:
        ...
        if (pkt.addr1.upper() in macprotectlist and not pkt.addr2.upper() in macprotectlist)\
                and time.time() > (lastassoc + duration):
            lastassoc = time.time()
            risk = "RISK: MEDIUM \n"
            message = pkt.sprintf("Association detected! \n Client %Dot11.addr2%  \n AP: "
                                  "%Dot11Elt.info% \n BSSID: %Dot11.addr1% \n ") + risk
            if notify and time.time() > (lastauth + duration):
                notifypopup(message)
            if verbose:
                print(message)
            if logging:
                slogger.warn('WIFIMON:' + message)
            if savecap:
                try:
                    writer = PcapWriter(filename, append=True)
                    writer.write(pkt)
        ...
        if (pkt.addr1.upper() in macprotectlist and not pkt.addr2.upper() in macprotectlist)\
                and time.time() > (lastauth + duration):
            lastauth = time.time()
            risk = "RISK: MEDIUM \n"
            ...
            if verbose:
                print(message)
            if notify and time.time() > (lastassoc + duration):
                notifypopup(message)
            if logging:
                slogger.warn('WIFIMON:' + message)
            if savecap:
        ...
        if pkt.info():
            if not ssidprotect or ssidprotect != pkt.info():
                return
        if (pkt.addr3.upper() in macprotectlist) and time.time() > (lastproberesp + duration):
            ...
            if verbose:
                print(message)
            if notify and time.time() > (lastproberesp + duration):
                notifypopup(message)
            if logging:
                slogger.warn('WIFIMON:' + message)
            if savecap:
...
def SetChannel(channels):
    ...
    cmd1 = 'iwconfig %s channel %s >/dev/null 2>&1' % (intfmon, channels)
    try:
        os.system(cmd0)
        os.system(cmd1)
        if verbose:
        ...
        if logging:
            slogger.debug("Error setting channel for %s" % intfmon)
...
if savecap:
    print("Capture option enabled: saved cap stored in: %s" % filename)
if logging:
    slogger.debug("WIFIMON: Starting scanning process in %s." % intfmon)

if verbose:
    print("Looking for suspicious packets in channel %s" % channel)
